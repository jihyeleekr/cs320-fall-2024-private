...
(* Checks if a type variable `x` occurs in the given type `ty`. Prevents infinite loops during substitution. *)
let rec occurs x ty =
  match ty with
  | TVar y -> x = y
...
(* Collects all free variables in a type `ty`. *)
...
(* Applies a substitution to a type `ty`. Substitution is a mapping of type variables to types. *)
...
(* Instantiates a type scheme by replacing bound type variables with fresh type variables. *)
...
(* let unique lst =
  let rec aux seen = function
    | [] -> []
    | x :: xs when List.mem x seen -> aux seen xs
    | x :: xs -> x :: aux (x :: seen) xs
  in
  aux [] lst *)
...
(* let env_free_vars env =
  let rec aux acc = function
    | [] -> acc
    | (_, Forall (_, ty)) :: rest -> aux (free_vars ty @ acc) rest
  in
  aux [] (Env.to_list env) *)

(* Generalizes a type `ty` by identifying variables not bound in the environment. *)
(* let generalize env ty =
  let env_vars = unique (env_free_vars env) in
  let ty_vars = unique (free_vars ty) in
  let generalized_vars = List.filter (fun v -> not (List.mem v env_vars)) ty_vars in
  Forall (generalized_vars, ty) *)

(* Unify Function *)
(* Attempts to unify a type `ty` with a list of constraints. Returns unified type if successful. *)
  let rec unify ty constraints =
    match constraints with
    | [] -> Some (Forall ([], ty))  (* Wrap the result in a `Forall`. *)
    ...
    let type_of (env : stc_env) (e : expr) : ty_scheme option =
      let rec infer env = function
        ...
        | PairMatch { matched; fst_name; snd_name; case } ->
          ...
          let env_extended = Env.add fst_name (Forall ([], fresh1)) (Env.add snd_name (Forall ([], fresh2)) env) in
          let t_case, c_case = infer env_extended case in
          (t_case, (t_matched, TPair (fresh1, fresh2)) :: c_matched @ c_case)
        | ListMatch { matched; hd_name; tl_name; cons_case; nil_case } ->
          let t_matched, c_matched = infer env matched in
          let fresh = TVar (gensym ()) in
          let env_extended = Env.add hd_name (Forall ([], fresh)) (Env.add tl_name (Forall ([], TList fresh)) env) in
          let t_cons_case, c_cons = infer env_extended cons_case in
          let t_nil_case, c_nil = infer env nil_case in
          (t_cons_case, (t_matched, TList fresh) :: (t_cons_case, t_nil_case) :: c_matched @ c_cons @ c_nil)
...
let rec eval_expr env expr : value =
  ...
  | App (e1, e2) -> (
      ...
      | VClos { env; name; arg; body } ->
          ...
          eval_expr env body
      | _ -> failwith "Fail with Application"
    )
  | Bop (Add, e1, e2) -> (
      match go e1, go e2 with
      | VInt m, VInt n -> VInt (m + n)
      | _ -> failwith "Fail with add"
    )
  | Bop (AddF, e1, e2) -> (
      match go e1, go e2 with
      | VFloat m, VFloat n -> VFloat (m +. n)
      | _ -> failwith "Fail with float add"
    )
  | Bop (Sub, e1, e2) -> (
      match go e1, go e2 with
      | VInt m, VInt n -> VInt (m - n)
      | _ -> failwith "Fail with sub"
    )
  | Bop (SubF, e1, e2) -> (
      match go e1, go e2 with
      | VFloat m, VFloat n -> VFloat (m -. n)
      | _ -> failwith "Fail with float sub"
    )
  | Bop (Mul, e1, e2) -> (
      match go e1, go e2 with
      | VInt m, VInt n -> VInt (m * n)
      | _ -> failwith "Fail with mul"
    )
  | Bop (MulF, e1, e2) -> (
      match go e1, go e2 with
      | VFloat m, VFloat n -> VFloat (m *. n)
      | _ -> failwith "Fail with float mul"
    )
  | Bop (Div, e1, e2) -> (
      match go e1, go e2 with
      | VInt _, VInt 0 -> raise DivByZero
      | VInt m, VInt n -> VInt (m / n)
      | _ -> failwith "Fail with div"
    )
  | Bop (DivF, e1, e2) -> (
      match go e1, go e2 with
      | VFloat m, VFloat n when n <> 0.0 -> VFloat (m /. n)
      | _ -> failwith "Fail with float div"
    )
  | Bop (Mod, e1, e2) -> (
      match go e1, go e2 with
  ...
  | Bop (PowF, e1, e2) -> (
      match go e1, go e2 with
      | VFloat m, VFloat n -> VFloat (m ** n)
      | _ -> failwith "Fail with float power"
    )
  | Bop (Eq, e1, e2) -> (
      match go e1, go e2 with
